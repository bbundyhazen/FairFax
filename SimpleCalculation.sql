    ...
    SELECT
        ...
        WHEN sc.CalculationType = '*' THEN
            CASE 
                ...
                WHEN CHARINDEX('MULTIPLY.NULLS', rd.CalculationTag) > 0 
                     AND COUNT(CASE WHEN rd.Value IS NULL THEN 1 END) > 0 THEN NULL
                -- MULTIPLY.ZEROES: If any value is 0, return 0
                WHEN CHARINDEX('MULTIPLY.ZEROES', rd.CalculationTag) > 0 
                     AND COUNT(CASE WHEN rd.Value = 0 THEN 1 END) > 0 THEN 0
                -- MULTIPLY.NONZEROES: Handle negative values properly
                ELSE 
                    EXP(SUM(LOG(ABS(NULLIF(rd.Value, 0))))) 
                    * CASE 
                        -- If an odd number of negative values, the result is negative
                        WHEN COUNT(CASE WHEN rd.Value < 0 THEN 1 END) % 2 = 1 THEN -1 
                        ELSE 1 
                    END
            END
        ...
        WHEN sc.CalculationType = '/' THEN
            CASE 
                ...
                WHEN CHARINDEX('DIVIDE.NULLS', rd.CalculationTag) > 0 
                     AND COUNT(CASE WHEN rd.Value IS NULL THEN 1 END) > 0 THEN NULL

                -- DIVIDE.UNDEFINED.NULLS: If any value is NULL or denominator is 0, return NULL
                WHEN CHARINDEX('DIVIDE.UNDEFINED.NULLS', rd.CalculationTag) > 0 
                     AND (COUNT(CASE WHEN rd.Value IS NULL THEN 1 END) > 0 
                          OR COUNT(CASE WHEN rd.Value = 0 THEN 1 END) > 0) THEN NULL

                -- DIVIDE.CHAINED.ZEROES: If any denominator in the chain is 0, return 0
                WHEN CHARINDEX('DIVIDE.CHAINED.ZEROES', rd.CalculationTag) > 0 
                     AND COUNT(CASE WHEN rd.Value = 0 THEN 1 END) > 0 THEN 0

                -- Default Division with proper sign handling
                ELSE
                    ...
                    /
                    -- Denominator: product of absolute values
                    NULLIF(
                        EXP(SUM(LOG(ABS(NULLIF(rd.Value, 0)))))
                        * CASE 
                            -- If an odd number of negative values in the denominator, adjust the sign
                            WHEN COUNT(CASE WHEN rd.Value < 0 THEN 1 END) % 2 = 1 THEN -1 
                            ELSE 1 
                          END, 
                        0
